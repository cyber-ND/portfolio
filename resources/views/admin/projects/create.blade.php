...
<body class="bg-black">
    <div class="relative min-h-screen bg-black overflow-hidden">
        ...
        <div class="relative max-w-md mx-auto mt-16 px-4 sm:px-8 py-8 bg-black/20 rounded-lg">
            <h2 class="text-2xl sm:text-3xl font-bold mb-6 text-white text-center">Create Project</h2>
            @if (session('success'))
            ...
            <form method="POST" action="{{ route('admin.projects.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title') }}" class="w-full px-3 py-2 bg-transparent border border-purple-400 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-purple-500 {{ $errors->has('title') ? 'border-red-500' : '' }}" required>
                </div>
                <div class="mb-4">
                    <label for="description" class="block text-white text-sm font-bold mb-2">Description</label>
                    <textarea id="description" name="description" class="w-full px-3 py-2 bg-transparent border border-purple-400 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-purple-500 {{ $errors->has('description') ? 'border-red-500' : '' }}" required>{{ old('description') }}</textarea>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-6">
                    ...
                    <input id="link" type="url" name="link" value="{{ old('link') }}" class="w-full px-3 py-2 bg-transparent border border-purple-400 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-purple-500 {{ $errors->has('link') ? 'border-red-500' : '' }}">
                </div>
                <div class="flex justify-center gap-4">
                    <button type="submit" class="px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white font-bold rounded-md transition-all duration-300">Create</button>
